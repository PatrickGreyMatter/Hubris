...
class NotesAndFavoritesController extends Controller
{
    ...
    {
        if (!Auth::check()) {
            return redirect()->route('login')->with('message', 'Vous devez vous connecter pour ajouter un film à votre librairie.');
        }
    ...
    {
        if (!Auth::check()) {
            return redirect()->route('login')->with('message', 'Vous devez vous connecter pour mettre une note.');
        }
